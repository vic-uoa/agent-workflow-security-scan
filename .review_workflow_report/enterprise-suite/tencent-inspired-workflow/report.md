...
- 节点/边：10 / 11
- Finding 数：103
- 严重等级：CRITICAL=6、HIGH=65、MEDIUM=30、LOW=1、INFO=1
- 证据状态：CONFIRMED=54、PROBABLE=37、COVERAGE_GAP=12
- 发布门禁：`FAIL`
...
- 动态用例：TC-b422495e9c0e, TC-bed377930035, TC-b0981fd71b46, TC-a439886058b6, TC-7fc9aec3fac1, TC-cdd9e47c6a71, TC-cfc6a962181a, TC-38bed1903338, TC-6505e0dfb07a, TC-7cba823f2e95, TC-00df2202370a
...
- 状态：`CONFIRMED, PROBABLE`
- 动态用例：TC-f5ee7cad2b09, TC-1b5b29477714, TC-caed4f42e78d, TC-1453fdccc43f, TC-a24e0df112cb, TC-86df90667ae7
...
- 状态：`CONFIRMED, PROBABLE`
- 动态用例：TC-f5ee7cad2b09, TC-1b5b29477714, TC-82db08834ff5, TC-412d145586a9, TC-a3ebce17ee94, TC-59e9deccc8bb, TC-ac57087d0787, TC-c3062591c771, TC-5a87afc0d52f, TC-b0981fd71b46, TC-a439886058b6, TC-7fc9aec3fac1, TC-cdd9e47c6a71, TC-cfc6a962181a, TC-38bed1903338, TC-6505e0dfb07a, TC-7cba823f2e95, TC-00df2202370a
...
- 状态：`CONFIRMED, PROBABLE`
- 动态用例：TC-b422495e9c0e, TC-bed377930035, TC-9269b138e9cb, TC-b59114204e7c
...
- 路径：`admin_tool → exfil`
- 状态：`PROBABLE`
- 动态用例：TC-caed4f42e78d, TC-1453fdccc43f, TC-a24e0df112cb, TC-86df90667ae7
...
### HIGH · TOOL-012, LLM-002

攻击族：general_workflow_security

- 路径：`memory → llm2`
- 状态：`CONFIRMED, PROBABLE`
- 动态用例：TC-d75a64e8fb5e, TC-50b0b00d95dc, TC-7ad33e37f436, TC-e086e64bf446, TC-b5f3aebaa656
...
- 状态：`CONFIRMED, PROBABLE`
- 动态用例：TC-82db08834ff5, TC-412d145586a9, TC-a3ebce17ee94, TC-59e9deccc8bb, TC-ac57087d0787, TC-c3062591c771, TC-5a87afc0d52f, TC-b0981fd71b46, TC-a439886058b6, TC-7fc9aec3fac1, TC-cdd9e47c6a71, TC-cfc6a962181a, TC-38bed1903338, TC-6505e0dfb07a, TC-7cba823f2e95, TC-00df2202370a
...
- 路径：`web → llm1`
- 状态：`CONFIRMED`
- 动态用例：TC-d75a64e8fb5e, TC-50b0b00d95dc, TC-7ad33e37f436
...
- 路径：`start → web`
- 状态：`CONFIRMED`
- 动态用例：TC-82db08834ff5, TC-412d145586a9, TC-a3ebce17ee94, TC-59e9deccc8bb, TC-ac57087d0787, TC-c3062591c771, TC-5a87afc0d52f
...
- 路径：`exfil`
- 状态：`CONFIRMED`
- 动态用例：TC-224f48c84b5d, TC-23eaf94ec198
...
- 路径：`llm2 → exfil`
- 状态：`CONFIRMED`
- 动态用例：TC-037787bd966d, TC-21103979333a, TC-22fd12983123, TC-3e0c7892bb3d
...
- 动态验证：`external_content_to_code_execution`
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。
...
- 动态验证：`inter_agent_message_injection`
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。
...
- 动态验证：`cross_user_memory_isolation`
- 修复建议：
  - 限制数据集和元数据范围，将检索内容视为不可信数据并保留来源。
...
- 动态验证：`persistent_memory_poisoning`
- 修复建议：
  - 限制数据集和元数据范围，将检索内容视为不可信数据并保留来源。
...
- 动态验证：`cross_tenant_object_access`
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。
...
- 动态验证：`human_agent_trust_exploit`
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [MEDIUM] TOOL-001 · 工具能力无法确定

工具能力无法从内部基线或 DSL 描述中确定。

- 状态：`COVERAGE_GAP`；置信度：1.00
- 节点：`memory`
- DSL 位置：`/workflow/graph/nodes/4`
- 证据：`FACT-c2386c898e53`
- 缺失上下文：需要在 internal-baseline.yml 登记工具能力和副作用。
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。
