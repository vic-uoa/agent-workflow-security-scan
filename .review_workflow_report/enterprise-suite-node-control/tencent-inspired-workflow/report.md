...
### [CRITICAL] TOOL-017 · 外部 HTTP 写入：网络与输出外发控制不足

责任节点“外部 HTTP 写入”在“网络与输出外发控制不足”方面存在 3 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`exfil`；控制域：`egress_control`
...
- 根因指纹：`RISK-3ad5158441e2`
- 关联规则（不重复计数）：TOOL-003, FLOW-009
- 建议动态测试：`ssrf`, `web_exfiltration_chain`（本次未执行）
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] FLOW-003 · 外部 HTTP 写入：高影响动作授权控制不足

责任节点“外部 HTTP 写入”在“高影响动作授权控制不足”方面存在 2 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`exfil`；控制域：`action_authorization`
- 代表路径：`start → admin_tool → exfil`；路径变体：2
- 合并证据实例：2
- DSL 位置：`/workflow/graph/nodes/0`, `/workflow/graph/nodes/6`, `/workflow/graph/nodes/7`, `/workflow/graph/nodes/1`
- 证据：`FACT-63cb48341a87`, `FACT-7bb959489e1c`
- 根因指纹：`RISK-1b0949096404`
- 建议动态测试：`source_to_high_impact_sink`（本次未执行）
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] FLOW-003 · 外部网页读取：高影响动作授权控制不足

不可信数据存在绕开确定性校验/审批到达高危工具的路径。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`web`；控制域：`action_authorization`
- 代表路径：`start → web`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/0`, `/workflow/graph/nodes/2`
- 证据：`FACT-82b543f035e7`
- 根因指纹：`RISK-1e931174d25a`
- 建议动态测试：`source_to_high_impact_sink`（本次未执行）
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] FLOW-011 · 执行 Agent：结构化数据契约不足

上游 Agent 的自由文本被下游 Agent 信任，缺少消息 Schema、来源身份或不可信指令隔离。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`llm2`；控制域：`structured_data_contract`
- 代表路径：`llm1 → llm2`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/3`, `/workflow/graph/nodes/5`
- 证据：`FACT-2f7fa4ea7d74`
- 根因指纹：`RISK-7762319f8fcc`
- 建议动态测试：`inter_agent_message_injection`（本次未执行）
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] FLOW-012 · 动态代码执行：失败处理与资源预算不足

责任节点“动态代码执行”在“失败处理与资源预算不足”方面存在 2 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`code`；控制域：`resilience_budget`
...
### [HIGH] FLOW-012 · 管理员资源更新工具：失败处理与资源预算不足

责任节点“管理员资源更新工具”在“失败处理与资源预算不足”方面存在 2 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`admin_tool`；控制域：`resilience_budget`
...
### [HIGH] FLOW-012 · 外部 HTTP 写入：失败处理与资源预算不足

责任节点“外部 HTTP 写入”在“失败处理与资源预算不足”方面存在 3 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`exfil`；控制域：`resilience_budget`
...
### [HIGH] FLOW-013 · 管理员资源更新工具：Agent 目标与停止边界不足

自主 Agent 接收不可信上下文并可影响高危能力，但 DSL 未声明目标锁定、停止条件或紧急停止控制。

- 状态：`PROBABLE`；置信度：0.82
- 责任节点：`admin_tool`；控制域：`agent_governance`
...
- 根因指纹：`RISK-979865bc0743`
- 关联规则（不重复计数）：LLM-001, LLM-002, IN-009
...
- 根因指纹：`RISK-64f9e7a7749b`
- 关联规则（不重复计数）：FLOW-004, TOOL-007
- 建议动态测试：`sensitive_data_exfiltration`（本次未执行）
- 修复建议：
  - 限制数据集和元数据范围，将检索内容视为不可信数据并保留来源。
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] KB-009 · 客户身份与病例知识库：外部内容信任边界不足

检索内容进入模型前缺少可识别的注入筛查或隔离控制。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`kb`；控制域：`untrusted_content_boundary`
- 代表路径：`kb`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/1`
- 证据：`FACT-640c7a7c0c1f`
- 根因指纹：`RISK-99966d7cc527`
- 建议动态测试：`rag_indirect_prompt_injection`（本次未执行）
- 修复建议：
  - 限制数据集和元数据范围，将检索内容视为不可信数据并保留来源。

### [HIGH] KB-011 · shared persistent memory store：记忆身份与隔离控制不足

责任节点“shared persistent memory store”在“记忆身份与隔离控制不足”方面存在 3 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`memory`；控制域：`memory_identity_scope`
- 代表路径：`memory`；路径变体：3
- 合并证据实例：3
...
### [HIGH] TOOL-002 · 管理员资源更新工具：高影响动作授权控制不足

责任节点“管理员资源更新工具”在“高影响动作授权控制不足”方面存在 6 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`admin_tool`；控制域：`action_authorization`
...
- 根因指纹：`RISK-58f60dc3a5aa`
- 关联规则（不重复计数）：TOOL-008, TOOL-015, TOOL-016, FLOW-003
...
### [HIGH] TOOL-002 · 动态代码执行：高影响动作授权控制不足

责任节点“动态代码执行”在“高影响动作授权控制不足”方面存在 6 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`code`；控制域：`action_authorization`
...
- 根因指纹：`RISK-72beb5f3940c`
- 关联规则（不重复计数）：TOOL-008, FLOW-003, LLM-008
...
### [HIGH] TOOL-003 · 外部网页读取：网络与输出外发控制不足

动态 URL/Host 缺少域名或地址 Allowlist。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`web`；控制域：`egress_control`
- 代表路径：`web`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/2`
- 证据：`FACT-ceabf24a45ad`
- 根因指纹：`RISK-2caaca7447f3`
- 建议动态测试：`ssrf`（本次未执行）
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [HIGH] TOOL-012 · 执行 Agent：外部内容信任边界不足

工具输出未经严格 Schema 验证进入 LLM 上下文。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`llm2`；控制域：`untrusted_content_boundary`
- 代表路径：`memory → llm2`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/4`, `/workflow/graph/nodes/5`
- 证据：`FACT-caa1117d6700`
- 根因指纹：`RISK-c8e1e8d3fa73`
- 建议动态测试：`tool_output_prompt_injection`（本次未执行）
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [HIGH] TOOL-012 · 主 Agent：外部内容信任边界不足

责任节点“主 Agent”在“外部内容信任边界不足”方面存在 5 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`llm1`；控制域：`untrusted_content_boundary`
...
- 根因指纹：`RISK-ec216158724a`
- 关联规则（不重复计数）：FLOW-005, KB-005, LLM-003
...
### [MEDIUM] LLM-009 · 主 Agent：失败处理与资源预算不足

LLM 节点缺少可识别的 Token、重试、超时或预算限制。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`llm1`；控制域：`resilience_budget`
- 代表路径：`llm1`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/3`
- 证据：`FACT-62743a962286`
- 根因指纹：`RISK-03b4ed027e74`
- 建议动态测试：`resource_budget`（本次未执行）
- 修复建议：
  - 将外部内容作为不可信数据隔离，并在模型输出进入行为层前执行确定性校验。

### [MEDIUM] LLM-009 · 执行 Agent：失败处理与资源预算不足

LLM 节点缺少可识别的 Token、重试、超时或预算限制。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`llm2`；控制域：`resilience_budget`
- 代表路径：`llm2`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/5`
- 证据：`FACT-53d25bcbde22`
- 根因指纹：`RISK-7bf206fa7394`
- 建议动态测试：`resource_budget`（本次未执行）
- 修复建议：
  - 将外部内容作为不可信数据隔离，并在模型输出进入行为层前执行确定性校验。

### [MEDIUM] OUT-007 · 安全验证结果：用户输出安全控制不足

责任节点“安全验证结果”在“用户输出安全控制不足”方面存在 3 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`answer`；控制域：`output_safety`
- 代表路径：`kb → llm1 → llm2 → answer`；路径变体：2
- 合并证据实例：3
- DSL 位置：`/workflow/graph/nodes/9`, `/workflow/graph/nodes/1`, `/workflow/graph/nodes/3`, `/workflow/graph/nodes/5`
- 证据：`FACT-5a4a7003a8b8`, `FACT-9f6c074eef19`, `FACT-d53997d341bc`
- 根因指纹：`RISK-1668512415bc`
- 关联规则（不重复计数）：OUT-004, OUT-010
- 建议动态测试：`rich_text_injection`, `human_agent_trust_exploit`（本次未执行）
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [MEDIUM] TOOL-011 · 管理员资源更新工具：结构化数据契约不足

工具输入缺少可识别的严格 Schema。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`admin_tool`；控制域：`structured_data_contract`
- 代表路径：`admin_tool`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/6`
- 证据：`FACT-daa2a662dcb1`
- 根因指纹：`RISK-616e8270ca3e`
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [MEDIUM] TOOL-011 · shared persistent memory store：结构化数据契约不足

工具输入缺少可识别的严格 Schema。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`memory`；控制域：`structured_data_contract`
- 代表路径：`memory`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/4`
- 证据：`FACT-725e7f279e5f`
- 根因指纹：`RISK-8e7151968d67`
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [MEDIUM] TOOL-011 · 外部网页读取：结构化数据契约不足

工具输入缺少可识别的严格 Schema。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`web`；控制域：`structured_data_contract`
- 代表路径：`web`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/2`
- 证据：`FACT-f2ea412bed87`
- 根因指纹：`RISK-db7b24a46b27`
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [MEDIUM] TOOL-013 · 外部网页读取：失败处理与资源预算不足

工具缺少可识别的超时设置。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`web`；控制域：`resilience_budget`
- 代表路径：`web`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/2`
- 证据：`FACT-889fdc91680f`
- 根因指纹：`RISK-88d460f46978`
- 建议动态测试：`tool_timeout`（本次未执行）
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。

### [MEDIUM] TOOL-013 · shared persistent memory store：失败处理与资源预算不足

工具缺少可识别的超时设置。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`memory`；控制域：`resilience_budget`
- 代表路径：`memory`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/4`
- 证据：`FACT-858355960425`
- 根因指纹：`RISK-aa8c43ce14d5`
- 建议动态测试：`tool_timeout`（本次未执行）
- 修复建议：
  - 对高危参数使用 Allowlist/Schema，并在副作用前设置不可绕过的审批或策略门。
