...
### [CRITICAL] FLOW-009 · Markdown回答：网络与输出外发控制不足
...
### [CRITICAL] FLOW-009 · 外部HTTP写入工具：网络与输出外发控制不足

责任节点“外部HTTP写入工具”在“网络与输出外发控制不足”方面存在 4 个规则或路径实例；主项按最强静态证据定级，具体影响见实例明细。

- 状态：`CONFIRMED`；置信度：1.00
- 责任节点：`http`；控制域：`egress_control`
...
- 根因指纹：`RISK-4bee44afe1bf`
- 建议动态测试：`source_to_high_impact_sink`（本次未执行）
- 修复建议：
  - 在风险路径的必经位置增加确定性控制，并验证不存在旁路。

### [HIGH] FLOW-004 · Markdown回答：敏感数据保护控制不足
...
### [HIGH] FLOW-005 · 决策模型：外部内容信任边界不足

知识库内容可经 LLM 影响高危工具，形成间接 Prompt Injection 攻击链。

- 状态：`PROBABLE`；置信度：0.90
- 责任节点：`llm`；控制域：`untrusted_content_boundary`
...
### [HIGH] KB-009 · 企业敏感知识库：外部内容信任边界不足
...
### [MEDIUM] LLM-009 · 决策模型：失败处理与资源预算不足

LLM 节点缺少可识别的 Token、重试、超时或预算限制。

- 状态：`PROBABLE`；置信度：0.80
- 责任节点：`llm`；控制域：`resilience_budget`
- 代表路径：`llm`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/2`
- 证据：`FACT-91101ff7dec6`
- 根因指纹：`RISK-3830078f85df`
- 建议动态测试：`resource_budget`（本次未执行）
- 修复建议：
  - 将外部内容作为不可信数据隔离，并在模型输出进入行为层前执行确定性校验。

### [MEDIUM] OUT-007 · Markdown回答：用户输出安全控制不足
...
### [MEDIUM] TOOL-013 · 外部HTTP写入工具：失败处理与资源预算不足

工具缺少可识别的超时设置。

- 状态：`PROBABLE`；置信度：0.85
- 责任节点：`http`；控制域：`resilience_budget`
- 代表路径：`http`；路径变体：1
- 合并证据实例：1
- DSL 位置：`/workflow/graph/nodes/3`
- 证据：`FACT-ad44783ba931`
- 根因指纹：`RISK-ff973669bdee`
