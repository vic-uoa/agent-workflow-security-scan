...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan.add_argument("--samples", type=Path, help="Optional seed override; by default the scanner synthesizes inputs from the DSL contract")
    scan.add_argument("--mode", choices=("structure-only", "assessment"), default="assessment", help="Assessment performs deterministic static analysis and automatically generates the input cluster")
    ...
    scan.add_argument("--output", required=True, type=Path, help="Output root; the scanner writes an HTML report and verified input-test-cluster JSON under <workflow filename>/, without other intermediate artifacts")
