...
4. **Generate the input cluster.** Validate confirmed seeds against declared start-node types, required fields, lengths and options. Derive positive, negative, boundary, metamorphic, and rule-targeted inert cases with field-aware mutation. Resolve a full control-flow route to each target, apply directly solvable branch constraints, and label unresolved computed conditions as `PARTIAL` instead of claiming reachability. Reject unchanged mutations. Deduplicate identical concrete inputs while retaining node/Finding route variants. Preserve lineage, machine-readable oracles, `route_status`, and `NOT_EXECUTED` status.
5. **Correlate and report.** Map a case to an attack path only when its Finding, target node and route variant match; shared rule IDs alone never establish the mapping. Produce the static report, attack surface, input cluster, quality gate, and sandbox test plan. Report planned, route-satisfiable and executed coverage separately. Unexecuted cases cannot confirm, reject, upgrade, or suppress a Finding.
...
- Require every derived case to differ from its seed and to carry a deterministic oracle. Mutation must target the applicable user-controlled field rather than the first scalar encountered.
- Treat `path_variants` based only on variable references as data flow. Use graph edges and branch handles for executable control-flow routes; record unsolved code-derived predicates in `missing_route_context`.
