...
复合外泄链要求同时具备：不可信来源、指令绑定、类型化敏感资产、同一模型上下文、类型化载荷路径、攻击者可观察 Sink，以及不存在覆盖全部路径的已验证强制脱敏。任一关键前提未知时只能形成候选或覆盖缺口；字段名候选不能形成完整外泄链。只有所有路径均经过可信注册或可静态验证的强制控制时才记录为 `MITIGATED`，普通 DLP 字段和无法验证实现的自声明控制不会抑制风险。Prompt 边界若进入条件、机器消费或自动决策输出，会按完整性影响提升为中高等级待复核项。
