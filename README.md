...
报告顶部改为展示总风险数、严重个数和高危个数。风险证据状态移入每条风险展开后的概述，折叠列表更便于快速浏览；完整工作流图支持点击打开大图，并可放大、缩小、还原和滚动查看。报告仍是无需联网的单个 HTML 文件。

## v0.11.0 校准

本轮加入 Python 调用与参数摘要，区分普通数据处理、动态执行、固定 URL 参数、会话保持和持久记忆；修正普通 POST/条件分支的风险越级，并补上 Jinja 别名和代码拼装后的提示词血缘。具体接口可通过操作员注册表声明只读或延迟执行语义。

变更、样本对照、接入方式与覆盖限制见 [语义校准说明](skills/agent-workflow-security-scan/references/semantic-calibration-20260908.md)。
...
复合外泄链要求同时具备：不可信来源、指令绑定、类型化敏感资产、同一模型上下文、类型化载荷路径、攻击者可观察 Sink，以及不存在覆盖全部路径的已验证强制脱敏。任一关键前提未知时只能形成候选或覆盖缺口；字段名候选不能形成完整外泄链。只有所有路径均经过可信注册或可静态验证的强制控制时才记录为 `MITIGATED`，普通 DLP 字段和无法验证实现的自声明控制不会抑制风险。用户、知识或工具内容若被直接插入 system/developer 消息，按已确认配置缺陷处理；模型是否真正被劫持由独立动态测试记录。进入具有安全影响的条件、自动决策或高后果动作时才提高严重度；普通展示分支不自动升级。
